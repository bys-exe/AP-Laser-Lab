    ...
    <div className="animate-stage space-y-8 pb-20">
      <header className="flex justify-between items-start border-b border-zinc-800 pb-6">
        <div className="space-y-1">
          ...
          <p className="text-zinc-500 text-sm font-bold uppercase tracking-widest">Qualitative and Quantitative Analysis of Transition Rates.</p>
        </div>
      </header>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-7 space-y-6">
          <div className="flex gap-1 bg-zinc-900 p-1 border border-zinc-800">
            {(['abs', 'spon', 'stim'] as const).map(k => (
              <button
                key={k}
                onClick={() => setActive(k)}
                className={`flex-1 py-3 text-[10px] font-black uppercase tracking-widest transition-all ${
                  active === k ? 'bg-cyan-500 text-black shadow-lg' : 'text-zinc-600 hover:text-zinc-400'
                }`}
              >
          ...
          <div className="bg-black border border-zinc-800 h-[400px] relative overflow-hidden flex items-center justify-center shadow-inner">
          ...
          <div className="bg-zinc-900/80 p-6 border border-zinc-800 space-y-4 shadow-xl">
             <h3 className="text-lg font-black text-cyan-500 uppercase tracking-tight">{processInfo[active].title}</h3>
             <p className="text-sm text-zinc-400 leading-relaxed font-bold uppercase tracking-tight">{processInfo[active].desc}</p>
          </div>
        </div>
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="bg-zinc-900 p-8 border border-zinc-800 space-y-6 shadow-xl">
             <div className="space-y-4">
                <div className="flex justify-between items-center">
                  <label className="text-[10px] font-black uppercase text-zinc-600 tracking-widest">Radiation Density ρ(ν)</label>
                  <span className="text-cyan-500 font-mono font-bold">{density} uJ/m³</span>
                </div>
                <div className="px-2 py-4 bg-black border border-zinc-800 shadow-inner">
                  <input 
                    type="range" 
                    value={density} 
             ...
             <div className="bg-black p-6 border border-zinc-800 space-y-4 shadow-inner">
                <h4 className="text-[9px] font-black uppercase text-zinc-600 tracking-widest">Statistical Probability Scaling</h4>
                <div className="h-[100px] w-full flex items-end overflow-hidden border-b border-l border-zinc-800 p-1">
                   <svg width="100%" height="100%" viewBox="0 0 300 100" preserveAspectRatio="none">
                      <path d={`M 0 100 L 300 ${100 - (density * 0.9)}`} fill="none" stroke="#06b6d4" strokeWidth="5" style={{ transition: 'd 0.3s ease' }} />
                   </svg>
                </div>
          ...
          <button onClick={onNext} className="w-full bg-white text-black font-black py-5 shadow-2xl hover:bg-cyan-500 transition-all text-xl">
